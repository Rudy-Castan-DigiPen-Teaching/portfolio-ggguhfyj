...
namespace demos
{
    D07GradientNoise::D07GradientNoise()
    {
        ...
        constexpr glm::vec3 eye_position{ -5.0f, 5.0f, 5.0f };
        constexpr glm::vec3 target_position{ 0.0f, 0.0f, 0.0f };
    ...
    void D07GradientNoise::setupNoiseFrameBuffer()
    {
        ...
        if (!noiseFrameBuffer.LoadWithSpecification(spec))
        {
            throw std::runtime_error{ "Failed to create gradient noise framebuffer" };
        }
    ...
    void D07GradientNoise::updateSpectatorCamera()
    {
        ...
        spectatorCamera.MoveDirection = glm::vec3(0.0f);
        ...
        if (float length = glm::dot(spectatorCamera.MoveDirection, spectatorCamera.MoveDirection); length > 0.0f)
        {
        ...
        if (spectatorCamera.Yaw + spectatorCamera.Pitch != 0.0f)
        {
