...
namespace demos
{
    gsl::owner<IDemo*> create_demo(Demos the_demo)
    {
            ...
            it != DemoInformation.end())
        {
            ...
            {
                util::log_error("Failed to create demo {}\n{}\nloading default Value Noise Demo", it->DisplayName, e.what());
                if (the_demo != Demos::ValueNoise)
                {
                    return new D06ValueNoise();
                }
                return new D01HelloQuad();
            }
        }
        util::log_debug("Unknown demo requested, loading default Value Noise Demo");
        return new D06ValueNoise();
    }
