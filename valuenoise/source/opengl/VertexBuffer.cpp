...
namespace opengl
{
    ...
    VertexBuffer::VertexBuffer(GLsizei size_in_bytes) : size(size_in_bytes)
    {
        ...
        GL::BufferData(GL_ARRAY_BUFFER, static_cast<GLsizeiptr>(size), no_data, GL_DYNAMIC_DRAW);
    ...
    {
        GL::BindBuffer(GL_ARRAY_BUFFER, buffer_handle);
        GL::BufferSubData(GL_ARRAY_BUFFER, static_cast<GLintptr>(starting_offset), static_cast<GLsizeiptr>(size_bytes), data);
        GL::BindBuffer(GL_ARRAY_BUFFER, 0);
    }
}
